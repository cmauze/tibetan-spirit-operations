...
version: "0.1.0"
category: operations
tags: [fulfillment, nepal, international]
...
3. IN TRANSIT
   ...
   -> Alert operations-manager if delay >3 days beyond nepal_eta
...
4. CUSTOMS CLEARANCE
   ...
   -> If customs holds shipment -> escalate to operations-manager + ceo

5. DELIVERED TO ASHEVILLE
   -> warehouse-manager confirms receipt and condition
...
**Important**: Handmade art (thangkas, statues) often qualifies for duty-free entry under HS 97xx. The operations-manager should obtain certificates of authenticity from the workshop to support this classification. When in doubt, use the higher-duty classification and flag for review.
...
- Automatically update inventory on receipt (warehouse-manager confirms first)
- File customs entries without operations-manager review
- Change HS codes without ceo approval (tariff classification has legal implications)

All inventory updates require human confirmation:
- warehouse-manager confirms physical receipt and condition
- operations-manager reviews customs documents before filing
- ceo approves any HS code changes or duty disputes

## Escalation Paths

| Situation | Route To | Method | SLA |
|-----------|----------|--------|-----|
| Shipment dispatched (routine update) | operations-manager | WhatsApp (Bahasa Indonesia) | Informational |
| Shipment 1-3 days overdue | operations-manager | WhatsApp (Bahasa Indonesia) | 24 hours |
| Shipment >7 days overdue | operations-manager + ceo | WhatsApp + Dashboard | 12 hours |
| Customs hold or inspection | operations-manager + ceo | WhatsApp + Dashboard | 12 hours |
| Customs duty dispute | ceo | Dashboard + email alert | 24 hours |
...
```json
{
  ...
  "escalation_target": "operations-manager | ceo_and_operations-manager | warehouse-manager | null",
  "phase": 1,
