...
description: Generate weekly margin reports from the product_margin_detail materialized view. Use this skill on the weekly cron (Monday 9 AM ET), when ceo requests an ad-hoc margin review, or when pricing-strategy needs current margin data. Breaks down profitability by SKU, category, and channel with trend indicators and below-floor alerts.
version: "0.1.0"
category: finance
tags: [margin, reporting, analysis]
...
Produce a clear, actionable margin report that tells the ceo exactly where the business is making and losing money. This skill reads pre-computed data from the `product_margin_detail` materialized view and transforms it into a report with:
...
The report is for the ceo's strategic decision-making. It must be accurate, concise, and highlight what needs attention.
...
2. [COGS] 8 SKUs still at "estimated" confidence — operations-manager to provide invoices
...
- ceo reviews the weekly report via dashboard
- Pricing changes go through the pricing-strategy skill with ceo approval
- COGS corrections go through cogs-tracking with operations-manager/ceo confirmation

## Escalation Paths

| Situation | Route To | Method | SLA |
|-----------|----------|--------|-----|
| Routine weekly report | ceo | Dashboard | Monday 9 AM ET |
| SKU with negative margin on any channel | ceo | Dashboard (flagged URGENT) | 24 hours |
| >5 SKUs below floor | ceo | Dashboard + email alert | 24 hours |
| COGS confidence "unknown" on >10% of catalog | ceo + operations-manager | Dashboard + WhatsApp | 1 week |
...
```json
{
  ...
  "action_items": [
    ...
    "8 SKUs at estimated COGS — operations-manager to provide invoices"
  ],
  "formatted_report": "...",
  "phase": 1,
  "confidence": 0.92
