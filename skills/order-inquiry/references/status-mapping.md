...
### Nepal-Sourced Items
Items handcrafted by Nepal artisan partners carry a 2–4 week lead time before fulfillment. When a customer inquires about an item with this lead time:
- Explain the artisan sourcing context positively — these items are made to order by skilled craftspeople
...
FBA orders are fulfilled and tracked through Amazon, not Shopify. Shopify fulfillment status will not reflect the true state.
- Do not use Shopify tracking data for FBA orders
- Direct the customer to their Amazon order confirmation for tracking
- If the customer purchased on tibetanspirit.com but the order routes to FBA, explain this clearly
...
- Ask the customer to locate their original confirmation email (subject: "Your Tibetan Spirit order")
- Try alternate lookups: different email address, guest vs. account order, potential typo in order number
- If still unresolved after two attempts, escalate to customer-service-lead for manual investigation
