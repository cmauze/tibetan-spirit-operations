...
description: Route and process domestic (US) orders for fulfillment by Fiona in Asheville, NC. Use this skill when a new Shopify order is received, when generating daily pick/pack lists, when selecting carriers and generating shipping labels, or when handling fulfillment exceptions (backorders, address issues, multi-item orders).
---
...
Process US domestic orders through Fiona's Asheville warehouse. This skill handles order routing, pick list generation, carrier selection, and exception handling.
...
1. Is the shipping address in Mexico/Latin America?
   → Route to Omar (fulfillment-mexico skill)
...
4. All other US orders → Fiona (this skill)
```
...
- If validation fails: flag for Fiona to review, do not auto-ship
...
- If `inventory_extended.total_on_hand` = 0 for any line item:
  ...
  - If ETA > 14 days or no pipeline: escalate to Jhoti for sourcing decision

### Oversized/Fragile Items
- Singing bowls >10 inches: require custom boxing, flag for Fiona
...
All actions require Fiona's confirmation via the dashboard. The skill presents her with a "ready to ship" queue each morning.
