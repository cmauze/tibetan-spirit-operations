...
description: Draft purchase orders, shipment inquiries, and quality issue communications for Nepal suppliers. Use this skill when inventory triggers a reorder, when Jhoti requests a new PO, when a shipment inquiry is needed, or when a quality issue must be reported to a supplier. Produces bilingual output (English PO for supplier + Bahasa Indonesia briefing for Jhoti).
---
...
4. **Brief Jhoti in formal Bahasa Indonesia** for every supplier action requiring her review
...
1. What triggered this communication?
   a. Reorder alert from inventory-management (reorder_trigger_qty breached)
      -> PURCHASE_ORDER
   b. Manual request from Jhoti or Chris
...
2. For PURCHASE_ORDER:
   ...
   d. Draft Bahasa Indonesia briefing for Jhoti
   e. Queue for Jhoti approval before sending

3. For SHIPMENT_INQUIRY:
   a. Query overdue shipments (see SQL below)
   b. Draft polite inquiry email to supplier
   c. Draft Bahasa Indonesia update for Jhoti with context
...
4. For QUALITY_ISSUE:
   ...
   c. Draft Bahasa Indonesia briefing for Jhoti
   d. If value >$200 or >10% of shipment affected -> escalate to Chris
```
...
## Bahasa Indonesia Briefing Template (for Jhoti)

Every supplier communication generates a briefing for Jhoti in formal Bahasa Indonesia:
...
{What Jhoti needs to review/approve, framed as suggestions}
...
- **Bahasa Indonesia translation**: Sonnet 4.6 -- formal register quality matters for Jhoti's trust

## Phase 1 Behavior

In Phase 1, this skill drafts all communications but does NOT send them:
- POs queued in Jhoti's approval queue (WhatsApp notification in Bahasa Indonesia)
- Supplier emails held as drafts until Jhoti approves
- Quality issues always require Jhoti review before sending to supplier
- Payment confirmations require Chris approval for amounts >$500

## Escalation Paths

| Situation | Route To | Method | SLA |
|-----------|----------|--------|-----|
| Standard reorder PO | Jhoti | WhatsApp (Bahasa Indonesia) | 24 hours |
| Urgent reorder (stockout imminent) | Jhoti + Chris | WhatsApp + Dashboard | 12 hours |
| Overdue shipment >7 days | Jhoti | WhatsApp (Bahasa Indonesia) | 24 hours |
| Overdue shipment >21 days | Jhoti + Chris | WhatsApp + Dashboard | 12 hours |
| Quality issue <$200 | Jhoti | WhatsApp (Bahasa Indonesia) | 24 hours |
| Quality issue >$200 | Chris + Jhoti | Dashboard + WhatsApp | 12 hours |
| New supplier onboarding | Chris (approval) -> Jhoti (relationship) | Dashboard | 1 week |
...
```json
{
  ...
  "escalation_target": "jhoti | chris_and_jhoti",
  "phase": 1,
