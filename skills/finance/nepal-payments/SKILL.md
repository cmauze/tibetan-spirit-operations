---
name: nepal-payments
description: Track NPR payments to Nepal suppliers using the supplier_payments table. Use this skill for recording new invoices, monitoring USD/NPR exchange rates, matching invoices to POs, managing payment schedules, and coordinating payment approval with Jhoti (Bahasa Indonesia) and Chris. Feeds exchange rate data to cogs-tracking for accurate COGS.
---
...
5. **Jhoti is briefed in formal Bahasa Indonesia** on all payment actions
6. **Chris approves** all payments before wire transfer
...
1. What triggered this skill?
   ...
   b. Payment due date approaching (within 7 days)
      -> PAYMENT_REMINDER: brief Jhoti, queue Chris approval
   c. Exchange rate check (daily cron)
      -> RATE_MONITOR: check for >3% movement, alert if significant
   d. Payment execution approved by Chris
...
3. For PAYMENT_REMINDER:
   ...
   d. Draft approval request for Chris
   e. Brief Jhoti with payment summary in Bahasa Indonesia

4. For RATE_MONITOR:
   a. Fetch current USD/NPR rate
   b. Compare to rate used in outstanding invoices
   c. If movement >3%: alert Chris with impact on outstanding payables
...
- Alert thresholds:
  - **>3% movement** from rate on outstanding invoices: notify Chris via dashboard
...
Include this in every payment approval request so Chris can make informed timing decisions.

## Bahasa Indonesia Payment Briefing (for Jhoti)
...
- **Payment approval drafting**: Sonnet 4.6 -- needs to explain FX impact clearly to Chris
...
- Invoice recording: Jhoti confirms the invoice matches the PO
- Payment scheduling: Chris approves all payments before wire transfer
- Exchange rate alerts: informational only, no automatic timing decisions
- No automatic wire transfers -- Chris initiates payments manually after approval

Payment approval flow:
1. Skill drafts payment request with FX analysis
2. Jhoti reviews invoice match (Bahasa Indonesia briefing)
3. Chris approves USD amount and timing
4. Chris executes wire transfer manually
...
|-----------|----------|--------|-----|
| New invoice recorded | Jhoti | WhatsApp (Bahasa Indonesia) | 24 hours |
| Payment due within 7 days | Jhoti -> Chris | WhatsApp -> Dashboard | 48 hours |
| Payment overdue | Chris | Dashboard + email | 24 hours |
| FX rate moved >3% | Chris | Dashboard | 48 hours |
| FX rate moved >5% | Chris | Dashboard (URGENT) | 24 hours |
| Invoice doesn't match PO | Jhoti | WhatsApp (Bahasa Indonesia) | 24 hours |
| Invoice >$2,000 USD | Chris | Dashboard + email | 24 hours |
| Supplier claims non-receipt of payment | Chris + Jhoti | Dashboard + WhatsApp | 12 hours |
...
```json
{
  ...
  "jhoti_briefing_id": "...",
  "escalation_target": "jhoti | chris | chris_and_jhoti | null",
  "phase": 1,
