...
The email automation system is live. Gmail rules handle the obvious patterns (known senders, keyword matches) without touching the LLM. Ambiguous emails route through Haiku for fast classification. Edge cases escalate to Sonnet for nuanced judgment. All classifications land in Gmail labels — the same 10 core labels plus project-specific labels that the system recommended and Chris approved.

Shortwave shows the organized labels on top of Gmail's native label system. The Chief of Staff posts a daily classification summary to Slack. Chris reviews and corrects misclassifications. Each correction feeds back: the system recommends new deterministic rules to replace LLM calls for patterns that repeat. Over weeks, the ratio shifts — more rules, fewer LLM calls, lower cost, higher accuracy.

**The experience:** Your email is automatically classified. Shortwave shows organized labels. You approve classifications in Slack. The system gets smarter over time as rules replace LLM for known patterns.
...
### Claude Code Custom Subagents Library

Build a library of project-specific subagents in `.claude/agents/` — security-reviewer, performance-auditor, docs-linter, dependency-checker, etc. Each subagent has a focused role, limited tools, and a specific model assignment (Haiku for linting, Sonnet for review, Opus for architecture decisions). The library grows organically as recurring review patterns emerge during development. **Consider after Phase 1 when patterns are established.**

### MCP Server for Cross-System Queries

Build a custom MCP server that queries across brain/ (Obsidian), Supabase, and Notion in a single interface. Unified search: "find everything related to X" returns vault notes, database records, and Notion pages ranked by relevance. Eliminates context-switching between systems during research and planning sessions. **Consider after Phase 2.**

### Notion to Obsidian Selective Sync

For pages that live in both places (org chart, company overview, key SOPs), build a one-way sync from Notion (team edits) to brain/ (Chris's reference copy). Notion remains the team-facing source of truth; brain/ gets a read-only mirror that stays current without manual updates. Uses Notion API polling on a schedule, with conflict detection that flags divergence for human review. **Consider after Phase 0B when the Notion/Obsidian boundary is established.**
...
- [ ] I understand the session management strategy (context budgets, /compact, /clear, document-and-clear)
- [ ] I understand the model routing strategy (Opus for planning, Sonnet for execution, Haiku for discovery)
- [ ] I understand the Paperclip fallback path (PM2 cron + cron_runner.py)
- [ ] I understand the email automation architecture (rules -> Haiku -> Sonnet -> human review)
